...
public class HeroPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public static final String[] DESCRIPTIONS = {"所有基础牌和英雄牌，效果+","抽牌+"};
    ...
    public float atDamageGive(float damage, DamageInfo.DamageType type, AbstractCard card) {
        return card.hasTag(CustomTags.HERO_CARD)? damage+3.0F:damage;
    }

    @Override
    public float atDamageReceive(float damage, DamageInfo.DamageType damageType) {
        return damage>1? damage-1.0F:0.0F;
    }

    @Override
    public float modifyBlock(float blockAmount, AbstractCard card) {
        return card.hasTag(CustomTags.HERO_CARD)? blockAmount+3.0F:blockAmount;
    }
